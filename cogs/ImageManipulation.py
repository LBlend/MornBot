...
class ImageManipulation(commands.Cog):
    ...
    async def pride(self, ctx, *, bruker: discord.Member=None):
        ...
        async with ctx.channel.typing():
            ...
            await bruker.avatar_url_as(format='png').save(fp=f'./assetstemp//{bruker.id}_raw.png')

            raw_image = Image.open(f'./assets/temp/{bruker.id}_raw.png')
            image_edit = raw_image.convert('RGBA')
            width, height = image_edit.size
            gay = Image.open('./assets/misc/gay.png')
            gay_edit = gay.convert('RGBA')
            gay_edit = gay_edit.resize((width, height), Image.ANTIALIAS)
            gay_edit.save(f'./assets/misc/gay.png')
            mask = Image.open(f'./assets/misc/gay.png', 'r')
            image_edit.paste(gay_edit, mask=mask)
            image_edit.save(f'./assets/temp/{bruker.id}_edit.png')

            f = discord.File(f'./assets/temp/{bruker.id}_edit.png')
            embed = discord.Embed()
    ...
    async def needsmorejpg(self, ctx, bilde=None):
        ...
        async with ctx.channel.typing():
            ...
            image_edit.save(f'./assets/temp/{ctx.author.id}_edit.jpg', quality=5)

            f = discord.File(f'./assets/temp/{ctx.author.id}_edit.jpg')
            ...
            try:
                ...
                remove(f'./assets/temp/{ctx.author.id}_edit.jpg')
            except:
                pass
    ...
    async def deepfry(self, ctx, bilde=None):
        ...
        async with ctx.channel.typing():
            ...
            if not await LBlend_utils.download_photo(ctx, link=bilde, max_file_size=8, meassurement_type='MB',
                                                     filepath=f'./assets/temp/{ctx.author.id}_raw.jpg'):
                return

            raw_image = Image.open(f'./assets/temp/{ctx.author.id}_raw.jpg')
            image_edit = raw_image.convert('RGB')
            ...
            try:
                remove(f'./assets/temp/{ctx.author.id}_raw.jpg')
                remove(f'./assets/temp/{ctx.author.id}_edit.png')
            except:
                pass
